...
def play_video(type=None, channel=None, id=None, **kwargs):
    ...
    if check_key(playdata['info'], 'start') and check_key(playdata['info'], 'end'):
        startT = datetime.datetime.fromtimestamp(playdata['info']['start'])
        startT = convert_datetime_timezone(startT, "UTC", "UTC")
        endT = datetime.datetime.fromtimestamp(playdata['info']['end'])
    ...
    if check_key(playdata['info'], 'image'):
        program_image_large = playdata['info']['image']

    if check_key(playdata['info'], 'participants'):
        if check_key(playdata['info']['participants'], 'Actor'):
            for castmember in playdata['info']['participants']['Actor']:
                cast.append(castmember)

        if check_key(playdata['info']['participants'], 'Director'):
            for directormember in playdata['info']['participants']['Director']:
                director.append(directormember)

        if check_key(playdata['info']['participants'], 'Scriptwriter'):
            for writermember in playdata['info']['participants']['Scriptwriter']:
                writer.append(writermember)

        if check_key(playdata['info']['participants'], 'Ad6'):
            for creditsmember in playdata['info']['participants']['Ad6']:
                credits.append(creditsmember)
