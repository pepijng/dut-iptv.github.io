...
def vod_series(label, description, image, image_large, seasons, mediagroupid=None, **kwargs):
    ...
    for season in seasons:
        ...
        items.append(plugin.Item(
            label = label,
            info = {'plot': description},
            art = {'thumb': image, 'fanart': image_large},
...
def play_video(type=None, channel=None, id=None, title=None, **kwargs):
    ...
    if type == "program":
        playdata = api.play_url(type='program', id=id)
        properties['seekTime'] = 1
    elif type == "vod":
        playdata = api.play_url(type='vod', id=id)
        properties['seekTime'] = 1
    elif type == "channel":
        playdata = api.play_url(type='channel', id=id)

        if playdata['type'] == "program":
            properties['seekTime'] = 1
    ...
    if check_key(playdata, 'info'):
        ...
        if title:
            label += title + ' - ' + playdata['info']['title']
        else:
            label += playdata['info']['title']
    ...
    if rows:
        ...
        for row in rows:
            ...
            if urldata and check_key(urldata, 'play_url') and check_key(urldata, 'locator'):
                path = plugin.url_for(play_video, type='channel', channel=channeldata['channel_id'], id=channeldata['channel_id'], title=None, _is_live=True)
                playable = True
