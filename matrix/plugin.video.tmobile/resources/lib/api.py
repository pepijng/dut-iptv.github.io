...
class API(object):
    ...
    def get_channel_data(self, rows, row):
        path = ADDON_PROFILE + os.sep + "images" + os.sep + str(rows[row]['id']) + ".png"

        if os.path.isfile(path):
            image = path
        else:
            image = rows[row]['logos']['guide']

        channeldata = {
            'channel_id': rows[row]['id'],
            'channel_number': int(rows[row]['displayOrder']) + 1,
            'description': '',
            'label': rows[row]['title'],
    ...
    def play_url(self, type, channel=None, id=None):
        ...
        if type == 'channel' and channel:
            channel_url = '{base_url}/api/guide/details/{channel}'.format(base_url=CONST_BASE_URL, channel=channel)
            ...
            program_url = '{base_url}/api/guide/details/{channel}/{id}'.format(base_url=CONST_BASE_URL, channel=channel, id=id)
        ...
        playdata = {'path': path, 'license': license, 'token': token, 'sessionid': sessionid, 'info': info}
