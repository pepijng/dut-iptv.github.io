...
class API(object):
    ...
    def get_channel_data(self, row):
        ...
        path = ADDON_PROFILE + os.sep + "images" + os.sep + str(row['metadata']['channelId']) + ".png"
    ...
    def play_url(self, type, channel=None, id=None, force=True):
        if not force == True and time.time() < (settings.getInt(key='_drm_token_age') + 300):
            playdata = {'path': settings.get(key='_renew_path'), 'license': '', 'token': settings.get(key='_renew_token'), 'type': '', 'info': ''}
            return playdata
        ...
        if type == 'channel':
            info_url = '{api_url}/TRAY/SEARCH/LIVE?maxResults=1&filter_airingTime=now&filter_channelIds={channel}&orderBy=airingStartTime&sortOrder=desc'.format(api_url=settings.get(key='_api_url'), channel=channel)
        ...
        if program_id:
            info_url = '{api_url}/CONTENT/DETAIL/{type}/{id}'.format(api_url=settings.get(key='_api_url'), type=typestr, id=program_id)
            data = self.download(url=info_url, type='get', code=[200], data=None, json_data=False, data_return=True, return_json=True, retry=True, check_data=True)
